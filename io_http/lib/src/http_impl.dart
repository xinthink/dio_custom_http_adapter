...
typedef VoidConsumer<T> = void Function(T);
...
class _CopyingBytesBuilder implements BytesBuilder {
  ...
  void add(List<int> bytes) {
    ...
    final required = _length + bytesLength;
...
class _HttpIncoming extends Stream<Uint8List> {
  ...
  @override
  StreamSubscription<Uint8List> listen(VoidConsumer<Uint8List> onData,
...
class _HttpRequest extends _HttpInboundMessage implements HttpRequest {
  ...
  _HttpRequest(this.response, _HttpIncoming _incoming, this._httpServer, this._httpConnection)
      : super(_incoming) {
  ...
  Uri get requestedUri {
    if (_requestedUri == null) {
      ...
      var scheme =
          proto != null ? proto.first : _httpConnection._socket is SecureSocket ? 'https' : 'http';
  ...
  HttpSession get session {
    if (_session != null) {
      if (_session._destroyed) {
        ...
        _session = null;
        // Create session object by calling recursive.
        return session; // ignore: recursive_getters
      }
...
class _HttpClientResponse extends _HttpInboundMessageListInt implements HttpClientResponse {
  @override
  List<RedirectInfo> get redirects => _httpRequest._responseRedirects;

  // The HttpClient this response belongs to.
  final IoHttpClient _httpClient;
  ...
  _HttpClientResponse(_HttpIncoming _incoming, this._httpRequest, this._httpClient)
  ...
  static HttpClientResponseCompressionState _getCompressionState(
      IoHttpClient httpClient, _HttpHeaders headers) {
    ...
    if (followLoops != true) {
      for (var redirect in redirects) {
        if (redirect.location == url) {
          return Future.error(RedirectException('Redirect loop detected', redirects));
        }
      }
    }
    return _httpClient._openUrlFromRequest(method, url, _httpRequest).then((request) {
  ...
  StreamSubscription<Uint8List> listen(VoidConsumer<Uint8List> onData,
    ...
    if (compressionState == HttpClientResponseCompressionState.decompressed) {
      stream = stream.cast<List<int>>().transform(gzip.decoder).transform(const _ToUint8List());
    }
  ...
  bool get _shouldAuthenticateProxy {
    ...
    final challenge = headers[HttpHeaders.proxyAuthenticateHeader];
  ...
  bool get _shouldAuthenticate {
    // Only try to authenticate if there is a challenge in the response.
    final challenge = headers[HttpHeaders.wwwAuthenticateHeader];
    return statusCode == HttpStatus.unauthorized && challenge != null && challenge.length == 1;
  }
  ...
  Future<HttpClientResponse> _authenticate(bool proxyAuth) {
    ...
    Future<HttpClientResponse> retry() {
      ...
      return drain().then((_) {
        return _httpClient
            ._openUrlFromRequest(_httpRequest.method, _httpRequest.uri, _httpRequest)
    ...
    Future requestAuthentication(_AuthenticationScheme scheme, String realm) {
      if (proxyAuth) {
        ...
        return _httpClient._authenticateProxy(proxy.host, proxy.port, scheme.toString(), realm);
      } else {
        if (_httpClient._authenticate == null) {
          return Future.value(false);
        }
        return _httpClient._authenticate(_httpRequest.uri, scheme.toString(), realm);
      }
    }

    final challenge = authChallenge();
    assert(challenge != null || challenge.length == 1);
    final header = _HeaderValue.parse(challenge[0], parameterSeparator: ',');
    final scheme = _AuthenticationScheme.fromString(header.value);
    final realm = header.parameters['realm'];
    ...
    if (cr != null) {
      ...
      if (cr.scheme == _AuthenticationScheme.DIGEST &&
        ...
        if (cr.nonce == null || cr.nonce == header.parameters['nonce']) {
          ...
          // If stale is true retry with nonce.
      ...
      : _uri = uri,
        headers = _HttpHeaders(protocolVersion,
            defaultPortForScheme:
                uri.scheme == 'https' ? HttpClient.defaultHttpsPort : HttpClient.defaultHttpPort,
...
class _HttpResponse extends _HttpOutboundMessage<HttpResponse> implements HttpResponse {
  int _statusCode = 200;
  ...
  _HttpResponse(Uri uri, String protocolVersion, _HttpOutgoing outgoing, HttpHeaders defaultHeaders,
      String serverHeader)
...
class _HttpClientRequest extends _HttpOutboundMessage<HttpClientResponse>
  ...
  final Completer<HttpClientResponse> _responseCompleter = Completer<HttpClientResponse>();
  ...
  _HttpClientRequest(_HttpOutgoing outgoing, Uri uri, this.method, this._proxy, this._httpClient,
      this._httpClientConnection, this._timeline)
      : uri = uri,
  ...
  Future<HttpClientResponse> get done {
    if (_response == null) {
      _response = Future.wait([_responseCompleter.future, super.done], eagerError: true)
          .then((list) => list[0]);
    }
    return _response;
  }
  ...
  void _onIncoming(_HttpIncoming incoming) {
    ...
    if (followRedirects && response.isRedirect) {
      if (response.redirects.length < maxRedirects) {
        ...
        future = response.drain().then<HttpClientResponse>((_) => response.redirect());
      } else {
    ...
    future.then((v) => _responseCompleter.complete(v), onError: _responseCompleter.completeError);
  }
...
class _HttpOutgoing implements StreamConsumer<List<int>> {
  ...
  Future writeHeaders({bool drainRequest: true, bool setOutgoing: true}) {
    ...
    if (outbound is _HttpResponse) {
      ...
      if (response._httpRequest._httpServer.autoCompress &&
        ...
        List acceptEncodings = response._httpRequest.headers[HttpHeaders.acceptEncodingHeader];
  ...
  Future addStream(Stream<List<int>> stream) {
    ...
    // Use stream so we are able to pause (see below listen). The
      ...
      if (chunked) {
        if (_gzip) {
          ...
          if (_gzipBufferLength > 0) {
            _gzipSink.add(
                Uint8List.view(_gzipBuffer.buffer, _gzipBuffer.offsetInBytes, _gzipBufferLength));
          }
          _gzipBuffer = null;
          _gzipSink.close();
          _gzipAdd = null;
  ...
  bool _ignoreError(error) =>
      (error is SocketException || error is TlsException) && outbound is HttpResponse;
  ...
  void _addGZipChunk(List<int> chunk, void add(List<int> data)) {
    ...
    if (chunk.length > _gzipBuffer.length - _gzipBufferLength) {
      add(Uint8List.view(_gzipBuffer.buffer, _gzipBuffer.offsetInBytes, _gzipBufferLength));
      _gzipBuffer = Uint8List(_OUTGOING_BUFFER_SIZE);
      _gzipBufferLength = 0;
    }
    if (chunk.length > _OUTGOING_BUFFER_SIZE) {
      ...
      _gzipBuffer.setRange(_gzipBufferLength, _gzipBufferLength + chunk.length, chunk);
...
class _HttpClientConnection {
  ...
  _HttpClientConnection(this.key, this._socket, this._httpClient,
    ...
    _subscription = _httpParser.listen((incoming) {
      ...
      if (_nextResponseCompleter != null) {
        _nextResponseCompleter.completeError(
            HttpException('Connection closed before response was received', uri: _currentUri));
        _nextResponseCompleter = null;
      }
      close();
    });
  ...
  _HttpClientRequest send(Uri uri, int port, String method, _Proxy proxy, TimelineTask timeline) {
    if (closed) {
    ...
    // Create request object, wrapping the outgoing connection.
    var request = _HttpClientRequest(outgoing, uri, method, proxy, _httpClient, this, timeline);
    ...
    if (proxy.isAuthenticated) {
      ...
      String auth = _CryptoUtils.bytesToBase64(utf8.encode('${proxy.username}:${proxy.password}'));
    ...
    _streamFuture = outgoing.done.then<Socket>((Socket s) {
      ...
      _nextResponseCompleter = Completer<_HttpIncoming>();
      ...
      _nextResponseCompleter.future.then((incoming) {
        ...
        // requests the client to start using a nonce for proxy
        // authentication.
        if (proxyCreds != null && proxyCreds.scheme == _AuthenticationScheme.DIGEST) {
          var authInfo = incoming.headers['proxy-authentication-info'];
          if (authInfo != null && authInfo.length == 1) {
            var header = _HeaderValue.parse(authInfo[0], parameterSeparator: ',');
        ...
        request._onIncoming(incoming);
          ...
          // element.
          .catchError((error) {
        throw HttpException('Connection closed before data was received', uri: uri);
  ...
  Future<Socket> detachSocket() {
    return _streamFuture.then((_) => _DetachedSocket(_socket, _httpParser.detachIncoming()));
  }
  ...
  void close() {
    closed = true;
    _httpClient._connectionClosed(this);
    _streamFuture.timeout(_httpClient.idleTimeout).then((_) => _socket.destroy());
  }

  Future<_HttpClientConnection> createProxyTunnel(String host, int port, _Proxy proxy,
      bool callback(X509Certificate certificate), TimelineTask timeline) {
    ...
    IoHttpClient._startRequestTimelineEvent(timeline, method, uri);
    _HttpClientRequest request = send(Uri(host: host, port: port), port, method, proxy, timeline);
    if (proxy.isAuthenticated) {
      // If the proxy configuration contains user information use that
      // for proxy basic authorization.
      String auth = _CryptoUtils.bytesToBase64(utf8.encode('${proxy.username}:${proxy.password}'));
    ...
    return request.close().then((response) {
      ...
      var socket = (response as _HttpClientResponse)._httpRequest._httpClientConnection._socket;
      return SecureSocket.secure(socket, host: host, context: _context, onBadCertificate: callback);
    }).then((secureSocket) {
      String key = _HttpClientConnection.makeKey(true, host, port);
      timeline?.instant('Proxy tunnel established');
      return _HttpClientConnection(key, secureSocket, request._httpClient, true);
    });
  }
...
class _ConnectionTarget {
  ...
  _ConnectionTarget(this.key, this.host, this.port, this.isSecure, this.context, [this.hostName]);
  ...
  Future<_ConnectionInfo> connect(
      String uriHost, int uriPort, _Proxy proxy, IoHttpClient client, TimelineTask timeline) {
    if (hasIdle) {
    ...
    final connectionTask = Socket.startConnect(host, port);
    _connecting++;
    return connectionTask.then((ConnectionTask<Socket> task) {
      _socketTasks.add(task);
      var socketFuture = task.socket;
      ...
      return socketFuture.then((socket) {
        if (isSecure && proxy.isDirect) {
          print('--- use secure socket Host=$hostName');
        }
        ...
        if (socket == null) {
          ...
          throw SocketException('HTTP connection timed out after ${connectionTimeout}, '
              'host: ${host}, port: ${port}');
        }
        _connecting--;
        socket.setOption(SocketOption.tcpNoDelay, true);
        final connection = _HttpClientConnection(key, socket, client, false, context);
        if (isSecure && !proxy.isDirect) {
          connection._dispose = true;
          return connection
              .createProxyTunnel(uriHost, uriPort, proxy, callback, timeline)
              .then((tunnel) {
            client._getConnectionTarget(uriHost, uriPort, true, hostName).addNewActive(tunnel);
            _socketTasks.remove(task);
...
/// A [HttpClient] implementation based on the dart:io package.
class IoHttpClient implements HttpClient {
  bool _closing = false;
  bool _closingForcefully = false;
  final Map<String, _ConnectionTarget> _connectionTargets = HashMap();
  ...
  factory IoHttpClient({SecurityContext context}) => IoHttpClient._(context);

  IoHttpClient._(this._context);
  ...
  Future<HttpClientRequest> open(String method, String host, int port, String path,
    ...
    return _openUrl(method, uri, hostName);
  }

  @override
  Future<HttpClientRequest> openUrl(String method, Uri url, [String hostName]) =>
      _openUrl(method, url, hostName);

  @override
  Future<HttpClientRequest> get(String host, int port, String path, [String hostName]) =>
      open('get', host, port, path, hostName);

  @override
  Future<HttpClientRequest> getUrl(Uri url, [String hostName]) => _openUrl('get', url, hostName);

  @override
  Future<HttpClientRequest> post(String host, int port, String path, [String hostName]) =>
      open('post', host, port, path, hostName);

  @override
  Future<HttpClientRequest> postUrl(Uri url, [String hostName]) => _openUrl('post', url);

  @override
  Future<HttpClientRequest> put(String host, int port, String path, [String hostName]) =>
      open('put', host, port, path, hostName);

  @override
  Future<HttpClientRequest> putUrl(Uri url, [String hostName]) => _openUrl('put', url, hostName);

  @override
  Future<HttpClientRequest> delete(String host, int port, String path, [String hostName]) =>
      open('delete', host, port, path, hostName);

  @override
  Future<HttpClientRequest> deleteUrl(Uri url, [String hostName]) =>
      _openUrl('delete', url, hostName);

  @override
  Future<HttpClientRequest> head(String host, int port, String path, [String hostName]) =>
      open('head', host, port, path, hostName);

  @override
  Future<HttpClientRequest> headUrl(Uri url, [String hostName]) => _openUrl('head', url, hostName);

  @override
  Future<HttpClientRequest> patch(String host, int port, String path, [String hostName]) =>
      open('patch', host, port, path, hostName);

  @override
  Future<HttpClientRequest> patchUrl(Uri url, [String hostName]) =>
      _openUrl('patch', url, hostName);
  ...
  void close({bool force: false}) {
    ...
    assert(!force || !_connectionTargets.values.any((s) => s._active.isNotEmpty));
  }
  ...
  static void _startRequestTimelineEvent(TimelineTask timeline, String method, Uri uri) {
  ...
  Future<_HttpClientRequest> _openUrl(String method, Uri uri, [String hostName]) {
    if (_closing) {
    ...
    int port = uri.port;
    if (port == 0) {
      port = isSecure ? HttpClient.defaultHttpsPort : HttpClient.defaultHttpPort;
    }
    ...
    return _getConnection(uri.host, port, proxyConf, isSecure, timeline, hostName).then(
      ...
      if (info.connection.closed) {
        return _getConnection(uri.host, port, proxyConf, isSecure, timeline, hostName).then(send);
      }
      return send(info);
    }, onError: (error) {
  ...
  Future<_HttpClientRequest> _openUrlFromRequest(
    ...
    // If the URI is relative (to either '/' or some sub-path),
  ...
  _ConnectionTarget _getConnectionTarget(String host, int port, bool isSecure, [String hostName]) {
    String key = _HttpClientConnection.makeKey(isSecure, host, port);
    return _connectionTargets.putIfAbsent(key, () {
      return _ConnectionTarget(key, host, port, isSecure, _context, hostName);
    });
  }

  // Get a _HttpClientConnection, from the matching _ConnectionTarget.
  Future<_ConnectionInfo> _getConnection(String uriHost, int uriPort, _ProxyConfiguration proxyConf,
      bool isSecure, TimelineTask timeline, String hostName) {
    ...
    Future<_ConnectionInfo> connect(error) {
      ...
      return _getConnectionTarget(host, port, isSecure, hostName)
          .connect(uriHost, uriPort, proxy, this, timeline)
          // On error, continue with next proxy.
          .catchError(connect);
    }

    return connect(HttpException('No proxies given'));
  }

  _SiteCredentials _findCredentials(Uri url, [_AuthenticationScheme scheme]) {
    // Look for credentials.
    _SiteCredentials cr = _credentials.fold(null, (_SiteCredentials prev, value) {
  ...
  static String _findProxyFromEnvironment(Uri url, Map<String, String> environment) {
    checkNoProxy(String option) {
      if (option == null) return null;
      Iterator<String> names = option.split(',').map((s) => s.trim()).iterator;
      while (names.moveNext()) {
        var name = names.current;
        if ((name.startsWith('[') && name.endsWith(']') && '[${url.host}]' == name) ||
...
class _HttpConnection extends LinkedListEntry<_HttpConnection> with _ServiceObject {
  ...
  static Map<int, _HttpConnection> _connections = HashMap<int, _HttpConnection>();
...
class _HttpServer extends Stream<HttpRequest> with _ServiceObject implements HttpServer {
  ...
  static Future<HttpServer> bindSecure(address, int port, SecurityContext context, int backlog,
      bool v6Only, bool requestClientCertificate, bool shared) {
...
class _Proxy {
  ...
  const _Proxy(this.host, this.port, this.username, this.password) : isDirect = false;
  const _Proxy.direct()
      : host = null,
        port = null,
        username = null,
...
class _HttpConnectionInfo implements HttpConnectionInfo {
  ...
  static _HttpConnectionInfo create(Socket socket) {
    ...
    try {
      _HttpConnectionInfo info = _HttpConnectionInfo();
      return info
...
class _SiteCredentials extends _Credentials {
  ...
  _SiteCredentials(this.uri, realm, _HttpClientCredentials creds) : super(creds, realm);

  bool applies(Uri uri, _AuthenticationScheme scheme) {
    if (scheme != null && credentials.scheme != scheme) return false;
    if (uri.host != this.uri.host) return false;
    int thisPort = this.uri.port == 0 ? HttpClient.defaultHttpPort : this.uri.port;
...
class _HttpClientBasicCredentials extends _HttpClientCredentials
  ...
  String authorization() {
    ...
    // http://tools.ietf.org/html/draft-reschke-basicauth-enc-06. For
    // now always use UTF-8 encoding.
    String auth = _CryptoUtils.bytesToBase64(utf8.encode('$username:$password'));
    return 'Basic $auth';
  }
...
class _HttpClientDigestCredentials extends _HttpClientCredentials
  ...
  String authorization(_Credentials credentials, _HttpClientRequest request) {
    ...
    if (credentials.qop == 'auth') {
      ...
      hasher..add(credentials.nonce.codeUnits)..add([_CharCode.COLON])..add(ha2.codeUnits);
    }
    ...
    if (qop == 'auth') {
      buffer..write(", qop='$qop'")..write(", cnonce='$cnonce'")..write(", nc='$nc'");
    }
  ...
  void authorize(_Credentials credentials, HttpClientRequest request) {
    request.headers.set(HttpHeaders.authorizationHeader, authorization(credentials, request));
  }

  void authorizeProxy(_ProxyCredentials credentials, HttpClientRequest request) {
    request.headers.set(HttpHeaders.proxyAuthorizationHeader, authorization(credentials, request));
  }
}
