...
class _HttpDetachedStreamSubscription implements StreamSubscription<Uint8List> {
  ...
  Future<T> asFuture<T>([T futureValue]) => _subscription.asFuture<T>(futureValue);
...
class _HttpDetachedIncoming extends Stream<Uint8List> {
  ...
  StreamSubscription<Uint8List> listen(void onData(Uint8List event),
    ...
    if (subscription != null) {
      ...
      return new Stream<Uint8List>.fromIterable([bufferedData])
...
class _HttpParser extends Stream<_HttpIncoming> {
    ...
    while (_buffer != null &&
      ...
      switch (_state) {
        ...
        case _State.METHOD_OR_RESPONSE_HTTP_VERSION:
          if (_httpVersionIndex < _Const.HTTP.length && byte == _Const.HTTP[_httpVersionIndex]) {
            // Continue parsing HTTP version.
            _httpVersionIndex++;
          } else if (_httpVersionIndex == _Const.HTTP.length && byte == _CharCode.SLASH) {
        ...
        case _State.RESPONSE_HTTP_VERSION:
          if (_httpVersionIndex < _Const.HTTP1DOT.length) {
            ...
            _httpVersionIndex++;
          } else if (_httpVersionIndex == _Const.HTTP1DOT.length && byte == _CharCode.ONE) {
            // HTTP/1.1 parsed.
            _httpVersion = _HttpVersion.HTTP11;
            _persistentConnection = true;
            _httpVersionIndex++;
          } else if (_httpVersionIndex == _Const.HTTP1DOT.length && byte == _CharCode.ZERO) {
            // HTTP/1.0 parsed.
        ...
        case _State.RESPONSE_LINE_REASON_PHRASE:
          if (byte == _CharCode.CR) {
            ...
            if (byte == _CharCode.CR || byte == _CharCode.LF) {
              throw HttpException("Invalid response, unexpected $byte in reason phrase");
            }
            _addWithValidation(_uriOrReasonPhrase, byte);
          }
          break;
        ...
        case _State.HEADER_VALUE_FOLD_OR_END:
          if (byte == _CharCode.SP || byte == _CharCode.HT) {
            ...
            if (headerField == HttpHeaders.connectionHeader) {
              ...
              final bool isUpgradeCode = (_statusCode == HttpStatus.upgradeRequired) ||
                  (_statusCode == HttpStatus.switchingProtocols);
              for (int i = 0; i < tokens.length; i++) {
                final bool isUpgrade =
                    _caseInsensitiveCompare("upgrade".codeUnits, tokens[i].codeUnits);
        ...
        case _State.BODY:
          ...
          Uint8List data =
              new Uint8List.view(_buffer.buffer, _buffer.offsetInBytes + _index, dataAvailable);
  ...
  void _createIncoming(int transferLength) {
    ...
    incoming = _incoming = new _HttpIncoming(_headers, transferLength, _bodyController.stream);
    _bodyPaused = true;
    _pauseStateChanged();
  }
