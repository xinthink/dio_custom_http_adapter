...
class _HttpSession implements HttpSession {
  ...
  Map<K, V> map<K, V>(MapEntry<K, V> transform(key, value)) => _data.map(transform);
  ...
  update(key, update(value), {ifAbsent()}) => _data.update(key, update, ifAbsent: ifAbsent);
...
class _HttpSessionManager {
  ...
  void _startTimer() {
    ...
    if (_head != null) {
      ...
      _timer = new Timer(new Duration(seconds: _sessionTimeout - seconds), _timerTimeout);
    }
  }
